...
This repository is transferred from [gdm-SynseneHealth](https://github.com/sensynehealth/polaris-articles-api) to Huma organisation as part of change in asset ownership 
<!-- Title - A concise title for the service that fits the pattern identified and in use across all services. -->
# Polaris Articles API
...
The Articles API is part of the Polaris platform (formerly DHOS). This service is responsible for managing news
articles to be read by patients.
...
<!-- markdown-swagger -->
 ...
 `/gdm/v1/image/{filename}`     | GET    | No    | Get an image by filename.                                               
 `/gdm/v1/article`              | GET    | No    | Retrieve all articles.                                                  
 `/gdm/v1/article`              | POST   | No    | Create a new article                                                    
 `/gdm/v1/article/{article_id}` | GET    | No    | Get an article by UUID                                                  
 `/gdm/v1/article/{article_id}` | DELETE | No    | Delete a article by UUID                                                
 `/gdm/v1/article/{article_id}` | PATCH  | No    | Update a article by UUID                                                
<!-- /markdown-swagger -->
...
## Database
Articles are stored in a Postgres database.
